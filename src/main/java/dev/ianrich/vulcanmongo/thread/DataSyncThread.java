    ...
    public void run() {
        ...
        Bukkit.getOnlinePlayers().forEach(player -> {
            if(player.hasPermission("basic.admin")) {
                player.sendMessage(ChatColor.BLUE + "[VulcanMongo] " + ChatColor.GRAY + "Saved " + LogManager.logs.size() + " logs!" + ChatColor.RESET + " (" + stopwatch.elapsed(TimeUnit.MILLISECONDS) + "ms)");
            }
        });
    }
